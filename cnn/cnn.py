...
class CNN:
    ...
    def train(self, training_dir: str, validation_dir: str, base_model: str, epochs: int = 1,
        ...
        training_datagen = tf.keras.preprocessing.image.ImageDataGenerator(# Estos es aumentar los datos para que tenga sentido
            preprocessing_function=self._preprocessing_function,
            rotation_range=30,# Rotacion 30 grados
            width_shift_range=0.2,
            height_shift_range=0.2,
            shear_range=0.2,# La estiro 
            zoom_range=0.2,# Hay un zoom
            horizontal_flip=True,  # Randomly flip half of the images horizontally# Giro horizontalmente
            fill_mode='nearest'  # Strategy used for filling in new pixels that appear after transforming images # Copio el pixel mas cercano
        )
        # Parámetros a introducir y probar: 
    ...
    def _add_output_layers(self, class_count: int, fc_layer_size: int = 1024):# Como entrada hago un modelo con todo lo de internet
        ...
        # Add new layers
        model.add(tf.keras.layers.Dense(fc_layer_size, activation='relu'))# Una capa densa con 1024 neuronas, hace cero a todos los valores negativos
        model.add(tf.keras.layers.Dropout(0.2))# 20% de las neuronas se apagan para evitar overfitting
